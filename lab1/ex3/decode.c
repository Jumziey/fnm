...
int main(int argc, char* argv[]){
	...
	for(i=0; i<SIZE/2-1; i++) {
		exponent = expf(-0.5*(((double)i - fc)/bw)*(((double)i - fc)/bw));
		REAL(grid,i) *= exponent;
		IMAG(grid,i) *= exponent;
		REAL(grid,SIZE-i) *= exponent;
		IMAG(grid,SIZE-i) *= exponent;
	}
