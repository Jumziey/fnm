...
//I'm waay to lazy for that ;) Btw aligned for tab = 2 spaces
...
volDepEval(sp sub, double *q)
{
	int i;
	//minus,normal, plus
	double pdir[3][9], rnn, *gamma;
	double h = 1E-20;// small compared to rnn
	double *omega[3];
		
	gamma = calloc(3, sizeof(double));
	for(i=0; i<3; i++)
		omega[i] = calloc(3,sizeof(double));
	
	rnn = sub.rnn;
	for(i=-1; i<2; i++) {
		sub.rnn = rnn+i*h;
		omega[i+1] = freqEval(sub, q);
	} 
	...
	for(i=0; i<3; i++) {
		...
		if(omega[1][i] == 0)
			return NULL;
		gamma[i] = (omega[2][i] - omega[0][i])/(pow(rnn+h,3)-pow(rnn-h,3));
		gamma[i] = fabs(gamma[i]*pow(rnn,3)/omega[1][i]);
	}
	return gamma;
}
